...
def move_enemy(grid, enemy_position, new_enemy_position):
    """
    ...
    Tip: what this function does is very similar to move_player, except with
    a "P" instead of an "M".
    ...
    x, y = enemy_position
    new_x, new_y = new_enemy_position
    grid[x][y] = ". "
    grid[new_x][new_y] = "P "
    return


def kill_enemy(grid, enemy_position, new_enemy_position):
    """
    ...
    x, y = enemy_position
    a, b = new_enemy_position

    if grid[a][b] == "* ":
        grid[x][y] = ". "
        grid[a][b] = "W "
    else:
        grid[a][b] = "P " #Could also use move function (to move normally)